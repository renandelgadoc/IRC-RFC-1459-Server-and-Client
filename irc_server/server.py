...
class Server:
    ...
    def list_channels(self, user, params):
        ...
        for channel in channels_user_count:
            data += channel + " - " + \
                str(channels_user_count[channel]) + " user(s)\n"
        ...
        if params[1][0] == '<':
            for a in self.users:
                if a.nickname == params[1]:
                    ...
                    a.conn.send(data.encode())
                    return
            raise Exception("ERR_NOSUCHNICK")

        elif params[1][0] in '&#':
            for a in self.users:
                if a.channel == params[1]:
                    data = user.nickname + " " + " ".join(params[2:])
                    a.conn.send(data.encode())
        
        return

    def who(self, user, params):
        ...
        for a in self.users:
            if a.channel == current_channel:
                channel_users += a.nickname + " "
        
        if channel_users == "":
            raise Exception("ERR_NOSUCHSCHANNEL")

        user.conn.send(channel_users.encode())
    ...
    def thread_cliente(self, conn) -> None:
        ...
        while True:
            try:
                ...
                data = "\nConfigure your user with the commands \"USER <username> <realname>\"and \"NICK <nickname>\", one at a time"
        ...
        while True:
            try:
                ...
                if instruction == "QUIT":
                    user.conn.close()
                    data = "server informing users that " + user.nickname + \
                        " closed his connection to the server"
                    for a in self.users:
                    ...
                    raise Exception("ERR UNKNOWNCOMMAND")
    ...
    def server_program(self) -> None:
        ...
        while True:
            ...
            x = threading.Thread(target=self.thread_cliente,
